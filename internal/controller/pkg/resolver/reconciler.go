...
// WithClient specifies the xpkg.Client the Reconciler should use to list
// package versions. The Client handles ImageConfig path rewriting and pull
...
func (r *Reconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) { //nolint:gocognit // we need to handle both pkg installation and upgrade scenarios now
	...
	lock := &v1beta1.Lock{}
	if err := r.kube.Get(ctx, req.NamespacedName, lock); err != nil {
